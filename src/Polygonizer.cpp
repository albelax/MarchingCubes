...
std::vector<glm::vec3> Polygonizer::getPoints()
{
  glm::vec3 point;
...
std::vector<float> Polygonizer::evaluate()
{
  float x, y, z;
...
glm::vec3 Polygonizer::lerp( float _isolevel, glm::vec3 _p1, glm::vec3 _p2, float _val1, float _val2 )
{
  float mu;
  glm::vec3 p;

  if ( std::abs( _isolevel - _val1) < 0.00001 )
    return( _p1 );
  if ( std::abs( _isolevel - _val2) < 0.00001 )
    return( _p2 );
  if ( std::abs( _val1 - _val2 ) < 0.00001 )
    return( _p1 );
  mu = ( _isolevel - _val1 ) / ( _val2 - _val1 );
